...
node 
{
   ...
   stage('Checkout')
   {
       ...
       sh 'oc set image --local=true -f Orchestration/deployment.yaml ${MS_NAME}=${DOCKER_REGISTRY}/mergedemo-dev-apps/${MS_NAME}:mergedemo-dev-apps --dry-run -o yaml >> Orchestration/deployment-mergedemo-dev.yaml'
  ...
  if(env.SECURITY_TESTING == 'True')
  {
   ...
   stage('Build and Tag Image for Dev')
   {
	   ...
	   FAILED_STAGE=env.STAGE_NAME
	   sh 'oc config use-context mergedemo-devops'
	  node('docker')
    {
        ...
        container('jnlp')
        {
            ...
            sh "docker build -t ${MS_NAME}:latest '/home/jenkins/workspace/${env.JOB_NAME}/${MS_NAME}'"
             sh 'docker tag ${MS_NAME}:latest ${DOCKER_REGISTRY}/${DOCKER_REPO}/$MS_NAME:mergedemo-dev-apps'
           sh 'docker login ${DOCKER_REGISTRY} --username $username  --password $password'
			sh 'docker push ${DOCKER_REGISTRY}/${DOCKER_REPO}/$MS_NAME:mergedemo-dev-apps'
   ...
   {   
       ...
       /*sh 'oc config use-context mergedemo-dev-apps'
       sh 'oc config view'
       sh 'oc apply -f Orchestration/deployment-mergedemo-dev.yaml -n=mergedemo-dev-apps'
       sh 'oc apply -f Orchestration/service.yaml -n=mergedemo-dev-apps' */
